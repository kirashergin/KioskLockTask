...
| 6 | Прозрачные system bars + `FLAG_LAYOUT_NO_LIMITS` | Контент рисуется под барами, кнопки невидимы |
| 7 | `immersive_mode_confirmations=confirmed` | Отключает обучающий popup "свайпните для выхода" |
...
На Android 12+ (API 31+) при свайпе от нижнего края экрана система может кратковременно показать полупрозрачный scrim (затемнение) в зоне навигационной панели. Это поведение контролируется SystemUI на уровне выше любых app-окон и не может быть полностью устранено через Device Owner API.

**Что сделано для минимизации:**
- Навигационные кнопки полностью прозрачны — видимых элементов нет
- Контент приложения рисуется под барами (`FLAG_LAYOUT_NO_LIMITS`)
- Бары мгновенно скрываются обратно (50мс задержка)
- `setLockTaskFeatures(0)` блокирует все действия из навигации

**Важно:** при тестировании через `adb shell input swipe` scrim появляется чаще, потому что ADB инжектит события в обход lock task restrictions. На реальном устройстве с пальцем `setLockTaskFeatures(0)` блокирует жест навигации более надёжно.

**Для полного устранения scrim** на конкретном устройстве:
- Кастомная прошивка с модифицированным SystemUI
- Samsung Knox / AOSP kiosk ROM
- Физический корпус киоска, закрывающий края экрана
